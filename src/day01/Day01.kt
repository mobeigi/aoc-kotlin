...
private object Day01 {
    ...
    private fun getLeftAndRightSpelledDigits(
        line: String
    ): Pair<IndexedValue<Int>?, IndexedValue<Int>?> {
        ...
        spelledDigits.forEach {
            ...
            if (
                index != -1 &&
                    (spelledDigitFirstIndex == null || index < spelledDigitFirstIndex!!.index)
            ) {
                spelledDigitFirstIndex = IndexedValue(index, it.second)
            }
            if (
                lastIndex != -1 &&
                    (spelledDigitLastIndex == null || lastIndex > spelledDigitLastIndex!!.index)
            ) {
